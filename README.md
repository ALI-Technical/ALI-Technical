<h1 align="center">Hi 👋, I'm ALI</h1>

<h3 align="center">Software Engineer • Backend & Full-Stack</h3>

<p align="center">
  Building production-grade applications, backend systems, and integrations with a focus on clean architecture and reliable software.
</p>

---

### 👨‍💻 About Me

* 💻 Software Engineer with **4 years of professional experience**
* 🚀 Specialized in **Node.js, TypeScript, Express.js, and NestJS**
* 🎨 Full-stack experience with **React, Next.js, Vue, Nuxt, and Angular**
* ☁️ Hands-on experience with **Azure, AWS, GitHub Actions, and Docker**
* 🔌 Experienced in building **REST APIs, integrations, authentication, data synchronization, and production systems**
* 🔍 Interested in **backend architecture, scalable systems, cloud technologies, and developer tooling**
* 🐍 Currently expanding my backend toolkit with **Python**

### 🛠️ Languages & Technologies

**Languages:**
TypeScript • JavaScript • SQL • Python

**Backend:**
Node.js • Express.js • NestJS • REST APIs • Socket.IO • BullMQ • Redis

**Frontend:**
React.js • Next.js • Vue.js • Nuxt.js • Angular • Tailwind CSS • Bootstrap • Material UI

**Databases:**
MongoDB • PostgreSQL • MySQL • Firebase

**Cloud & DevOps:**
Azure • AWS (EC2, S3) • Docker • GitHub Actions • CI/CD

**Tools & Integrations:**
Zendesk • Azure AI Foundry • SearchKit • Git • GitHub • SonarQube • ESLint

### 🌱 Currently Exploring

**Python • AI Integrations • Cloud Architecture • Backend Systems**

### 📫 Connect With Me

* 💼 LinkedIn: [linkedin.com/in/ali-technical](https://linkedin.com/in/ali-technical)
* 📧 Email: **[ali.aa251104@gmail.com](mailto:ali.aa251104@gmail.com)**
* 🕸️ Portfolio: **[made-by-ali.vercel.app](https://made-by-ali.vercel.app/)**

---

<p align="center">
  <i>Think like an architect. Ship like an engineer.</i>
</p>
